...
# ============================================================================
# M3-cockpit: Transfer Evaluation (Cockpit Backend)
# ============================================================================

def run_cockpit_transfer_stage(
    data_dir: Path,
    output_dir: Path,
    device: str = "cuda",
) -> Dict:
    """
    M3 for cockpit: Collect natural traces from held-out cockpit tasks
    and evaluate CMV transfer.

    Uses COCKPIT_HELD_OUT_TASKS (10 tasks not seen during M1 training).
    Runs 3-branch replay on each → generates labeled transfer events.
    Then evaluates CMV (trained on M1 data) on these new events.
    """
    from cockpit.integration import (
        CockpitExecutor, CockpitReplayEngine, start_cockpit_server, CockpitClient,
    )
    from cockpit.validators import CockpitCompositeValidator
    from cockpit.task_definitions import COCKPIT_HELD_OUT_TASKS
    from .cmv_model import CMVModel, SigLIPFeatureExtractor, PostconditionTokenizer, FixedDelayBaseline
    from .cmv_trainer import load_events_from_dir, load_vlm_scores
    from .vlm_judge import VLMJudge
    from .evaluator import transfer_evaluation
    ...
    logger.info("=" * 60)
    logger.info("HybridStress Cockpit Transfer Stage (M3)")
    logger.info("=" * 60)
    ...
    # Save initial snapshot for condition isolation
    client.reset()
    client.save_snapshot("initial")

    # ── Step 1: Collect transfer events (full fault protocol) ──────────
    # Same 10-condition protocol as M1: 1 clean + 3 faults × 3 severities
    # This ensures boundary failures are created for the detector to detect.
    transfer_events_path = output_dir / "transfer_events"
    if transfer_events_path.exists() and list(transfer_events_path.glob("*.json")):
        logger.info("Transfer events already exist, loading...")
        natural_events = load_events_from_dir(transfer_events_path)
    else:
        transfer_events_path.mkdir(parents=True, exist_ok=True)
        logger.info(f"Collecting transfer traces from {len(COCKPIT_HELD_OUT_TASKS)} held-out tasks × 10 conditions...")
        ...
        natural_events = []
        for task_idx, task in enumerate(COCKPIT_HELD_OUT_TASKS):
            task_id = task["task_id"]
            conditions = generate_all_conditions(task_id)
            logger.info(f"  [{task_idx+1}/{len(COCKPIT_HELD_OUT_TASKS)}] {task_id} ({len(conditions)} conditions)")

            for cond_idx, (fault_type, fault_severity) in enumerate(conditions):
                fault_config = FaultConfig.from_type_severity(fault_type, fault_severity)
                condition_name = f"{fault_type.value}_{fault_severity.value}"

                # Restore clean state before each condition
                client.restore_snapshot("initial")
                snapshot_id = f"transfer_{task_id}_{cond_idx}"
                client.save_snapshot(snapshot_id)
                ...
                try:
                    ...
                    natural_events.append(event)
                    event.save(transfer_events_path / f"{event.event_id}.json")
                    logger.info(f"    [{cond_idx+1}/{len(conditions)}] {condition_name}: {event.label.value}")
                except Exception as e:
                    logger.error(f"    [{cond_idx+1}/{len(conditions)}] {condition_name} FAILED: {e}")

        logger.info(f"Collected {len(natural_events)} transfer switch events")

    if not natural_events:
        logger.warning("No transfer events collected. M3 cannot proceed.")
        return {"status": "no_events", "n_events": 0}
    ...
    # For cockpit, natural traces have replay-based ground truth
    # (3-branch labels) so human annotations are not required.
    detector_scores = {
        "vlm_judge": vlm_scores,
        "cmv": cmv_scores,
        "fixed_delay": fixed_scores,
    ...
    comparison = transfer_evaluation(natural_events, detector_scores, human_annotations=None)
    ...
    c2 = comparison.get("c2_results", {})
    logger.info("=" * 60)
    logger.info("M3 (Cockpit) COMPLETE")
    logger.info(
        f"C2: CMV AUPRC={c2.get('cmv_auprc', 0):.4f}, "
        f"gap={c2.get('gap_pct', 0):.1f}% — "
        f"{'PASSED' if c2.get('c2_overall_passed') else 'FAILED'}"
...
# ============================================================================
# M4-cockpit: Recovery Utility (Cockpit Backend)
# ============================================================================

def run_cockpit_utility_stage(
    data_dir: Path,
    output_dir: Path,
    device: str = "cuda",
) -> Dict:
    """
    M4 for cockpit: Recovery evaluation (C3, exploratory).
    Compare baseline vs blind-retry vs CMV-triggered recovery.
    """
    from cockpit.integration import (
        CockpitExecutor, CockpitReplayEngine, start_cockpit_server, CockpitClient,
    )
    from cockpit.validators import CockpitCompositeValidator
    from cockpit.screenshot import capture_screenshot_cockpit
    from cockpit.task_definitions import COCKPIT_TASKS
    from .cmv_model import CMVModel, SigLIPFeatureExtractor, PostconditionTokenizer
    from .recovery import RecoveryProtocol
    from .evaluator import recovery_evaluation
    ...
    logger.info("=" * 60)
    logger.info("HybridStress Cockpit Utility Stage (M4)")
    logger.info("=" * 60)
    ...
    # Inject moderate faults to create failures for recovery to act on
    fault_config = FaultConfig.from_type_severity(
        FaultType.PHANTOM_ACK, FaultSeverity.MODERATE
    )
    ...
    for task_idx, task in enumerate(tasks):
        ...
        # ── Run 1: Baseline (with fault, no recovery) ──────────────
        try:
            client.reset()
            client.save_snapshot("_pre_action")
            result = executor.execute_task_hybrid(task, fault_config=fault_config)
            post_path = str(output_dir / f"baseline_{task_id}.png")
            capture_screenshot_cockpit(post_path, cockpit_url)
            validator.set_screenshot(post_path)
            outcome, _ = validator.validate_all(task["postconditions"])
            baseline_results[task_id] = outcome.value == "success"
        except Exception as e:
        ...
        try:
            client.reset()
            client.save_snapshot("_pre_action")
            executor.execute_task_hybrid(task, fault_config=fault_config)
            # Blind retry: always re-execute via API
            executor.execute_task_api_only(task)
            post_path = str(output_dir / f"blind_{task_id}.png")
            capture_screenshot_cockpit(post_path, cockpit_url)
            validator.set_screenshot(post_path)
            outcome, _ = validator.validate_all(task["postconditions"])
        ...
        try:
            ...
            pre_path = str(output_dir / f"cmv_pre_{task_id}.png")
            capture_screenshot_cockpit(pre_path, cockpit_url)
            ...
            # Recovery: if CMV detects inconsistency, re-execute via API
            if cmv_score > 0.5:
                logger.info(f"  CMV detected inconsistency ({cmv_score:.3f}), recovering...")
                executor.execute_task_api_only(task)

            final_path = str(output_dir / f"cmv_final_{task_id}.png")
            capture_screenshot_cockpit(final_path, cockpit_url)
            validator.set_screenshot(final_path)
            outcome, _ = validator.validate_all(task["postconditions"])
            cmv_recovery_results[task_id] = outcome.value == "success"
        except Exception as e:
    ...
    logger.info("=" * 60)
    logger.info("M4 (Cockpit) COMPLETE")
    rates = eval_result["success_rates"]
    delta = eval_result["cmv_vs_baseline"]
    logger.info(
        f"C3: baseline={rates['baseline']:.2%}, cmv={rates['cmv_recovery']:.2%}, "
