...
class SetGearRequest(BaseModel):
    gear: str  # P, R, N, D
...
@router.post("/gear")
def set_gear(req: SetGearRequest):
    sm = get_state_manager()
    gear = req.gear.upper()
    if gear not in {"P", "R", "N", "D"}:
        return {"status": "error", "message": f"Unsupported gear: {req.gear}"}
    sm.update("vehicle", gear=gear)
    return {"status": "ok", "gear": gear}


@router.post("/ota_start")
def start_ota():
    sm = get_state_manager()
    sm.update("vehicle", ota_state="downloading", current_screen="vehicle_home")
    sm.set_active_app("vehicle")
    return {"status": "ok", "ota_state": "downloading"}


@router.post("/adas_calibrate")
def start_adas_calibration():
    sm = get_state_manager()
    sm.update(
        "vehicle",
        adas_calibration_state="calibrating",
        current_screen="vehicle_home",
    )
    sm.set_active_app("vehicle")
    return {"status": "ok", "adas_calibration_state": "calibrating"}
